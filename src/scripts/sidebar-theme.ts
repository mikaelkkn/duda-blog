...
const isLongPage = () =>
  /^(?:\/(?:textos|memo)(?:\/|$))/.test(window.location.pathname);
...
const setReaderDisabled = (disabled: boolean) => {
  ...
  if (disabled) {
    readerBtn.setAttribute('title', 'Modo leitura');
    readerBtn.setAttribute('aria-label', 'Modo leitura');
...
const applyReader = (on: boolean) => {
  ...
  if (readerExit) {
    readerExit.setAttribute('aria-label', 'Sair da leitura');
    readerExit.setAttribute('title', 'Sair da leitura');
  }
