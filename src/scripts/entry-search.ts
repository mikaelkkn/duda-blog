...
if (!root) {
  ...
  const getStatusPrefix = (query: string, totalMatches: number) => {
    if (query && activeTagLabel) {
      return `${totalMatches} resultado(s) com a tag #${activeTagLabel}`;
    }
    if (query) {
      return totalMatches === 0 ? 'Nenhum resultado encontrado' : `${totalMatches} resultado(s)`;
    }
    return '';
  };
  ...
  const updateStatusForMatches = (query: string, totalMatches: number, visibleMatches: number) => {
    ...
    if (visibleMatches === totalMatches) {
      setStatus(query && !activeTagKey ? `${totalMatches} resultado(s)` : prefix);
      return;
    }
    if (visibleMatches === 0) {
      setStatus(`${prefix} — nenhum nesta página`);
      return;
    }
    setStatus(`${prefix} — mostrando ${visibleMatches} desta página`);
  };
  ...
  const setDegradedMode = () => {
    ...
    showAllItems();
    syncSections(false);
    setStatus('Erro ao carregar busca');
  };
  ...
  const indexLoader = createJsonIndexLoader<IndexItem>({
    ...
    onPending: () => {
      setStatus('Carregando...', { visible: false });
    },
    onResolved: (data) => {
      indexHay = new Map(
        data.map((item) => [
